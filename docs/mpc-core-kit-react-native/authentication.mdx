...
Call `init()` first (after the `Bridge` is ready). If `status` is already `LOGGED_IN` or
`REQUIRED_SHARE`, do not login again — the instance was restored from a session. `loginWithJWT`
throws if the instance is already logged in or rehydrated. When `status` is `REQUIRED_SHARE`, call
`inputFactorKey` with a device, backup, or security-question factor.
...
login function. OAuth login is not available in `react-native` mode.

New vs existing user is decided by whether metadata exists for that postbox key.

- **New user** (no metadata): creates tKey and the first factor, then a session. The default hashed
  (cloud) factor results in `LOGGED_IN` after one JWT. With `disableHashedFactorKey`, a device factor
  is stored locally; you may still need `inputFactorKey`. `importTssKey` and
  `registerExistingSFAKey` only work for new users.
- **Existing user** (metadata exists): hashed factor still valid → `LOGGED_IN`. MFA enabled or
  `disableHashedFactorKey` → `REQUIRED_SHARE` until `inputFactorKey`.
...
| `prefetchTssPublicKeys?`  | Number of TSS public keys to prefetch (`0`–`3`). Set this to the number of factors you will create; use `0` for existing users. Default is `1`.                                                                                                                                                                                                                                                                           |
| `registerExistingSFAKey?` | New users only: convert an SFA user to TSS. Do not combine with `importTssKey`. v1 SFA users are not supported. Default value is `false`. Once the SFA key is imported, users won't be able to access their account using the SFA SDK.                                                                                                                                                                                    |
...
interface JWTLoginParams {
   ...
   * Cannot be combined with importTssKey. v1 SFA users are not supported.
   */
...
if (coreKitInstance.status === COREKIT_STATUS.REQUIRED_SHARE) {
  const hex = await coreKitInstance.getDeviceFactor(); // or mnemonicToKey(...) / securityQuestion.recoverFactor(coreKitInstance, answer)
...
`registerExistingSFAKey` parameter. Do not combine it with `importTssKey`. v1 SFA users are not
supported. Once the SFA key is imported, users will no longer be able to access their account
through the SFA SDK.
...
// Here network can be "sapphire_mainnet" or "sapphire_devnet", since MPC doesn't support
// legacy networks.
