...
`sign(data, hashed?)` takes a `Buffer`. `hashed` defaults to `false` (the SDK hashes with keccak256).
secp256k1 returns a 65-byte `r||s||v`. The JavaScript-only `precompute_secp256k1()` and
`setPreSigningHook` APIs are not available through the React Native facade.
...
const address = base58.encode(Uint8Array.from(await coreKitInstance.getPubKeyEd25519()));
```
