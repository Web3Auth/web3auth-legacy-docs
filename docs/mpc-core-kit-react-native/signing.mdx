...
[EthereumSigningProvider documentation](/mpc-core-kit-react-native/providers/evm).
...
both secp256k1 and ed25519 curve. Using `TssDklsLib` this function will return ECDSA signature and
using `TssFrostLib`, it will return EDDSA signature.

`sign(data, hashed?, precompute?)` takes a `Buffer`. `hashed` defaults to `false` (the SDK hashes
with keccak256). secp256k1 returns a 65-byte `r||s||v`. You can warm TSS with
`precompute_secp256k1()` and pass the result into the next `sign` — it is one-shot.

`setPreSigningHook` can abort signing if the hook returns `{ success: false }`.
`makeEthereumSigner(coreKitInstance)` wraps `sign(..., true)` as `{ sign(msgHash), getPublic() }`
for EVM providers.
...
// Warm TSS before the user signs to reduce latency. One-shot.
const pre = await coreKitInstance.precompute_secp256k1();
await coreKitInstance.sign(msg, false, pre);

coreKitInstance.setPreSigningHook(async ({ data, hashed }) => {
  return { success: true };
});
```
...
To sign the transaction for ed25519 curve, you can use the `sign` method. Using `TssFrostLib`, this
function returns an EdDSA signature.
