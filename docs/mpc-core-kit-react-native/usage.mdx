...
recover the account. Source the hex from `getDeviceFactor()`, `mnemonicToKey`, or another securely
stored factor key.
...
You can get the factor pubs with `await getTssFactorPub()`, or from
`(await getKeyDetails()).shareDescriptions`.
...
const pubs = await coreKitInstance.getTssFactorPub();
...
```ts
const pubKeyPoint = await coreKitInstance.getPubKeyPoint();
```
...
Security Question management and recovery are not currently supported through the React Native
Core Kit facade. The JavaScript `TssSecurityQuestion` helper requires access to
`mpcCoreKit.tKey`, which `Web3AuthMPCCoreKitRN` does not expose.

Do not pass a React Native Core Kit instance to `TssSecurityQuestion`. Use a supported factor
workflow, such as `getDeviceFactor()` or a securely stored recovery factor, until a native
Security Question API is available.
