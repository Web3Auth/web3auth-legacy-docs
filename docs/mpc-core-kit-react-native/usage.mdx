...
| --------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| [loginWithJWT](#login-with-jwt)               | Logs in the user with a JWT-based ID Token. Throws if already logged in or rehydrated.                                          |
| [inputFactorKey](#login-with-existing-factor) | This method can be used to login with the exisitng factor key, for instance when threshold is not satisfied (`REQUIRED_SHARE`). |
| [logout](#logout)                             | Ends the session and re-inits. Status is `INITIALIZED` (no rehydrate).                                                          |

:::info Get help with Authentication

Check out the [**Authentication**](./authentication) section of the SDK Reference to learn more
...
| [setManualSync](#set-manual-sync)    | Flushes pending writes, then turns manual sync on or off.                                                                                                                                                |
...
| [createFactor](#create-factor)                 | Creates a new TSS Factor for the user's account to increase. You can configure the type of factor, it can either be a device factor, or backup factor. Returns the factor hex. Max 10 factors.                       |
| [deleteFactor](#delete-factor)                 | This method allows you to deletes a TSS factor for the user. Cannot delete the last factor or the one currently in use.                                                                                              |
| [getTssFactorPub](#get-tss-factor-pub)         | Returns compressed hex public keys of all factors. Use these with `deleteFactor`.                                                                                                                                    |
...
| [setDeviceFactor](#set-device-factor)          | Save a device factor to `storage`. Throws if one exists unless `replace` is true.                                                                                                                                    |
...
| [getPubKeyPoint](#get-pubkey-point)         | Same key as a tKey `Point`.                                                                                                                                                                                                                |
...
These reconstruct the private key. Not recommended unless the user explicitly wants to export.

| Method Name                                          | Description                                                                      |
| ---------------------------------------------------- | -------------------------------------------------------------------------------- |
| [_UNSAFE_exportTssKey](#export-tss-key)              | secp256k1 private key hex for the current account index.                         |
| [_UNSAFE_exportTssEd25519Seed](#export-ed25519-seed) | ed25519 seed. Needs a client-generated or imported key.                          |
| [_UNSAFE_recoverTssKey](#recover-tss-key)            | Reconstruct TSS key from DEVICE **and** RECOVERY factors. Same type twice fails. |

### Additional Helper Methods

| Method Name                       | Description                                                                                   |
| --------------------------------- | --------------------------------------------------------------------------------------------- |
| [setSecurityQuestion](#set-security-question) | Creates a security question factor for the user's account.                                    |
| [changeSecurityQuestion](#change-security-question) | Changes the security question and answer for the account.                                      |
| [deleteSecurityQuestion](#delete-security-question-share) | Deletes the security question factor.                                                         |
| [recoverFactor](#get-factor-key-from-security-question) | Recovers the security question factor from the answer.                                        |
| [getQuestion](#get-security-question-for-users-account) | Retrieves the security question for the account.                                               |
...
| [generateFactorKey](#generate-factor-key) | Random factor `{ private: BN, pub: Point }`.                                                  |
| [sigToRSV](#sig-to-rsv)                   | Split a 65-byte secp256k1 signature into `{ r, s, v }`.                                       |
| [parseToken](#parse-token)                | Decode a JWT payload.                                                                         |
...
[authentication section](./authentication) for more
details.

### Login With Existing Factor

This method can be used to input the factor key when `status === REQUIRED_SHARE`. For example, if
user's device factor is not present, this method can be used to input the back up factor key, and
recover the account. Source the hex from `getDeviceFactor()`, `mnemonicToKey`, or
`TssSecurityQuestion.recoverFactor`.
If the factor key is valid, the SDK reconstructs tKey and creates a session.

The factor is always checked against metadata `factorEncs` before it is accepted. Please make sure
to convert your factor key to BN before passing it to this method.
...
This method logs out the user, invalidates the session, and re-inits without rehydrate. Status is
`INITIALIZED`.
...
Flushes pending metadata writes, then turns manual sync on or off.

```tsx
await coreKitInstance.setManualSync(true);
```
...
hashed factor must still exist. The method takes in `enableMFAParams` which allows you to configure
the backup factor of your choice. You can also opt to not create a backup factor by setting
`recoveryFactor` to `false`. If `recoveryFactor` is `false`, the return value is `undefined`.

If `manualSync` has pending edits, call `commitChanges()` first. After `enableMFA`, call
`commitChanges()` again when `manualSync` is `true`.
...
// Required when the instance was created with manualSync: true
await coreKitInstance.commitChanges();
...
```tsx
import { generateFactorKey, keyToMnemonic } from "@web3auth/react-native-mpc-core-kit";

const factorKey = generateFactorKey();

// focus-start
const factorHex = await coreKitInstance.createFactor({
  shareType: TssShareType.RECOVERY,
  factorKey: factorKey.private,
});
// focus-end
const mnemonic = keyToMnemonic(factorHex);
// Required when the instance was created with manualSync: true
await coreKitInstance.commitChanges();
```
...
respective to the factor pub provided for the user's account. You cannot delete the last factor or
the one currently in use. Pass `factorKey` to also delete its metadata backup.

You can get the factor pubs with `getTssFactorPub()`, or from `getKeyDetails().shareDescriptions`.
...
```tsx
import { factorKeyCurve } from "@web3auth/react-native-mpc-core-kit";
import { Point } from "@tkey/common-types";

const pubs = coreKitInstance.getTssFactorPub();
...
Save a device factor to `storage`. Throws if one already exists unless `replace` is `true`. When
`disableHashedFactorKey` is enabled, first login and session rehydrate persist the device factor
automatically.

```tsx
await coreKitInstance.setDeviceFactor(factorKey); // throws if one exists
await coreKitInstance.setDeviceFactor(factorKey, true); // replace
```
...
Same public key as a tKey `Point`.

```ts
const pubKeyPoint = coreKitInstance.getPubKeyPoint();
```
...
| `keyType`           | Curve in use: secp256k1 or ed25519.                                                                                                                                  |

## Export

These methods reconstruct the private key. Use them only when the user explicitly wants to export.

### Export TSS Key

Exports the secp256k1 private key hex for the current account index.

```tsx
const keyHex = await coreKitInstance._UNSAFE_exportTssKey();
```

### Export Ed25519 Seed

Exports the ed25519 seed. Requires `useClientGeneratedTSSKey` or an imported key.
...
Reconstructs the TSS key from DEVICE **and** RECOVERY factor hex values. Passing two factors of the
same type fails.

```tsx
const keyHex = await coreKitInstance._UNSAFE_recoverTssKey([deviceHex, recoveryHex]);
```

## Security Question

The `TssSecurityQuestion` helper manages a security question factor. Import it from
`@web3auth/mpc-core-kit`; the React Native SDK reuses this helper for security question recovery.

:::note

The user must meet the required factor threshold before creating, changing, or deleting a security
question factor. If `manualSync` is enabled, call `commitChanges()` after each successful change.

:::

### Set Security Question

Creates a security question recovery factor for the user's account.

```tsx
import { TssSecurityQuestion, TssShareType } from "@web3auth/mpc-core-kit";
...
Changes the security question and answer. The current answer is required.
...
Deletes the security question factor. The user must be logged in with the required factor threshold.
...
When `coreKitInstance.status === COREKIT_STATUS.REQUIRED_SHARE`, recover the factor key with the
security question answer and pass it to `inputFactorKey`.
...
const securityQuestion = new TssSecurityQuestion();
const factorKeyHex = await securityQuestion.recoverFactor(coreKitInstance, "YOUR_ANSWER");

if (factorKeyHex) {
  await coreKitInstance.inputFactorKey(new BN(factorKeyHex, "hex"));
}
```

### Get Security Question for User's Account

Retrieves the security question after the user has completed authentication.
...
Creates a random factor key pair.

```tsx
import { generateFactorKey } from "@web3auth/react-native-mpc-core-kit";

const { private: factorKey, pub } = generateFactorKey();
```

### Sig to RSV

Splits a 65-byte secp256k1 signature (`r||s||v`) into `{ r, s, v }`.

```tsx
import { sigToRSV } from "@web3auth/react-native-mpc-core-kit";

const { r, s, v } = sigToRSV(signature);
```
...
```tsx
import { parseToken } from "@web3auth/react-native-mpc-core-kit";
