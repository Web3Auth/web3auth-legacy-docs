...
`loginWithOAuth` is not available in `nodejs` or `react-native` `uxMode`. It throws if the instance
is already logged in or rehydrated.

In **redirect** mode the function returns immediately; `init()` or `handleRedirectResult()` finishes
login. `importTssKey` and `registerExistingSFAKey` are not allowed in redirect mode.
...
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `importTssKey?`           | New users only, popup mode only. The TSS key to import an existing account.                                                                                                                                                                           |
| `registerExistingSFAKey?` | New users only, popup mode only. Convert an SFA user to TSS. Do not combine with `importTssKey`. v1 SFA users are not supported. Default value is `false`. Once the SFA key is imported, users won't be able to access their account using the SFA SDK. |
...
through the SFA SDK. `importTssKey` and `registerExistingSFAKey` are not allowed in redirect mode
and only apply to new users. Do not combine them. v1 SFA users are not supported.
...
file. `loginWithOAuth` returns immediately; finish login with `init()` (default
`handleRedirectResult: true`) or call `handleRedirectResult()` yourself.

For example, if `baseUrl` is `https://your-domain.com` and `redirectPathName` is `auth` then the
user will be redirected to the `https://your-domain.com/auth` page after logging in. Call `init()`
on that page — it completes the OAuth redirect when `handleRedirectResult` is `true` (the default).

```tsx
await coreKitInstance.init();
```

If you set `handleRedirectResult: false`, `init()` skips the hash. Check for `#state` or
`#access_token` yourself, then call `handleRedirectResult()`.

```tsx
await coreKitInstance.init({ handleRedirectResult: false, rehydrate: true });
if (
