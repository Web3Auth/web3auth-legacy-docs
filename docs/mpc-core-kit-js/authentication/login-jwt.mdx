...
login function. Use this method for custom auth, Node, and React Native.

`loginWithJWT` throws if the instance is already logged in or rehydrated. After `init()`, check
`coreKitInstance.status` — if it is `LOGGED_IN` or `REQUIRED_SHARE`, do not call login again.
...
```tsx
import { COREKIT_STATUS, JWTLoginParams } from "@web3auth/mpc-core-kit";
...
through the SFA SDK. Do not combine `registerExistingSFAKey` with `importTssKey`. v1 SFA users are
not supported.
