...
Call `init()` first. If `status` is already `LOGGED_IN` or `REQUIRED_SHARE`, do not login again —
the instance was restored from a session or an OAuth redirect. Login methods throw if the instance
is already logged in or rehydrated. When `status` is `REQUIRED_SHARE`, call `inputFactorKey` with a
device, backup, or security-question factor.
