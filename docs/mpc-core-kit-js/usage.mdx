...
recover the account. Source the hex from `getDeviceFactor()`, `mnemonicToKey`, or `recoverFactor`.
...
`recoveryFactor` to `false`. If `recoveryFactor` is `false`, the return value is an empty string.
