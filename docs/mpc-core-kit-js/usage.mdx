...
| ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| [loginWithJWT](#login-with-jwt)                 | Logs in the user with a JWT-based ID Token. We recommend using this method to authenticate users for better performance. Throws if already logged in or rehydrated. |
| [loginWithOAuth](#login-with-oauth)             | Logs in the user using OAuth Flow, this function underneaths uses the CustomAuth flow to authenticate the user. Throws if already logged in or rehydrated.           |
| [handleRedirectResult](#handle-redirect-result) | Handles the redirect result from OAuth Flow. `init()` already does this when `handleRedirectResult` is true.                                                         |
...
| [MemoryStorage](#memory-storage)                          | In-memory storage for tests or Node.                                                                                                                                    |
...
providers with 0Auth login flow. Not available in `nodejs` or `react-native` mode. Throws if the
instance is already logged in or rehydrated. In redirect mode the function returns immediately;
`init()` or `handleRedirectResult()` finishes login. Please refer to the
...
By default, you won't require to use this method since `init()` handles the redirect result
internally. You can set the `handleRedirectResult` to false during initialization, and use this
...
```tsx
import { sigToRSV } from "@web3auth/mpc-core-kit";
...
In-memory storage for tests or Node.

```tsx
import { MemoryStorage, Web3AuthMPCCoreKit } from "@web3auth/mpc-core-kit";
